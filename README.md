...
La categoría independiente Art on EVM (`/art-on-evm`) usa `/api/transient`, limitada al perfil NikoAlerce (ID público 13262), con paginación y caché de cinco minutos. Muestra obras creadas, su colección y red; cada tarjeta abre su página en Transient. No intenta comprarlas mediante la wallet Tezos. El enlace anterior `/art-on-tezos?tab=transient` redirige permanentemente a la nueva categoría; Art on Tezos conserva únicamente Works y Sidequest.

Las tarjetas de Tezos cargan vistas previas estáticas cerca del viewport. `/api/thumbnail` reduce imágenes IPFS a WebP de hasta 640 px, decodifica solo el primer fotograma y permite cachear el resultado por CID. Solo consulta gateways fijos, limita descarga a 32 MiB y decodificación a 40 megapíxeles, y devuelve errores sin caché. Los videos/MP4 optimizados se cargan al pasar el mouse; los originales se conservan en el visor. Sharp 0.35.4 se incluye como dependencia de producción.

Transient conserva la proporción completa y la animación de sus miniaturas. Las ocho vistas previas actuales se sirven como MP4 en bucle con poster WebP, sin recortar: 384 × 480, 150 fotogramas, 12 segundos cada una. Se reproducen cerca del viewport y se pausan al salir. `node scripts/optimize-transient-previews.mjs` las regenera con ffmpeg y el sitio local en ejecución; el manifiesto se indexa por URI de origen. Para nuevas obras sin versión local se usa el CDN animado de Transient (`n=-1`), manteniendo el tamaño natural. Los archivos publicados usan nombres derivados del contenido de la URL de origen.
