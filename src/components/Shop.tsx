    ...
    >
      <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-6 mb-12">
        <div className="relative">
          ...
          <div className="text-[11px] tracking-[0.8em] mb-4 font-black text-glitch-terra relative z-10 uppercase">
            // PHYSICAL_GOODS_SYNC
          </div>
        ...
        <div className="text-[10px] tracking-[0.3em] text-ash space-y-1 border-l border-glitch-terra/30 pl-4">
          ...
          <div className="flex justify-between gap-6">
            <span>STOCK</span>
            <span className="text-glitch-terra">LIMITED</span>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 md:gap-4">
        {products.map((p) => (
          <article
            key={p.id}
            className="group relative bg-ink border border-white/5 hover:border-glitch-terra/60 transition-colors"
          >
            <div className="aspect-[4/5] relative overflow-hidden bg-black grid place-items-center">
              <div className="text-[18rem] leading-none text-white/5 select-none group-hover:text-glitch-terra/10 transition-colors font-mono">
                {p.emoji}
              </div>
              ...
              <div
                ...
                style={{
                  background:
                    "repeating-linear-gradient(0deg, rgba(196,122,90,0.04) 0 1px, transparent 1px 3px)",
                  mixBlendMode: "screen",
                }}
              />
            </div>
            ...
            <div className="p-3 border-t border-white/5">
              <div className="flex items-baseline justify-between gap-2">
                <h3 className="text-xs font-medium text-bone group-hover:text-glitch-terra transition-colors truncate">
                  {p.title}
                </h3>
              ...
              <button
                ...
                className="mt-4 w-full text-[11px] tracking-[0.3em] uppercase border border-white/10 px-3 py-2 hover:border-glitch-terra hover:text-glitch-terra disabled:opacity-40 disabled:hover:border-white/10 disabled:hover:text-ash transition-colors"
              >
                {p.status === "available"
                  ? "ADD_TO_CART"
                  : p.status === "soon"
